...
(() => {
  ...
  async function ivOpenCachedFile(record){
    ...
    const externalBtn=host.querySelector('[data-iv-open-external]');
    body.innerHTML=''; title.textContent=name;
    externalBtn.style.display = isPdf ? 'inline-block' : 'none';
    externalBtn.onclick = () => ivOpenWithExternalApp(record);
    if(isPdf){
  ...
  // ── "Open with another app" (Android) ──────────────────────────────
  // Writes the already-downloaded/cached blob to real on-device storage
  // (so it works fully offline — no network involved at any point), then
  // hands a proper content:// URI to the system chooser via FileProvider
  // (@capacitor/filesystem + @capacitor-community/file-opener handle the
  // FileProvider/content:// URI + FLAG_GRANT_READ_URI_PERMISSION plumbing
  // natively — no file:// URI is ever used).
  function ivBlobToBase64(blob){
  ...
  async function ivWriteBlobToFilesystem(blob,name){
    const { Filesystem }=window.Capacitor.Plugins || {};
    if(!Filesystem) return null;
    const base64=await ivBlobToBase64(blob);
    const safeName=safeDownloadName(name||'file.pdf').replace(/\s+/g,'_');
    const path='iv_files/'+Date.now()+'_'+safeName;
    const res=await Filesystem.writeFile({ path, data:base64, directory:'DATA', recursive:true });
    return res?.uri || null;
  }
  async function ivPersistNativeUri(key,uri){
    try{
      const db=await ivOpenCacheDb();
      if(!db||!key) return;
      const tx=db.transaction(IV_CACHE_STORE,'readwrite');
      const store=tx.objectStore(IV_CACHE_STORE);
      const getReq=store.get(key);
      getReq.onsuccess=()=>{ const row=getReq.result; if(row){ row.nativeUri=uri; store.put(row); } };
    }catch(_){}
  }
  async function ivOpenWithExternalApp(record){
    const isOffline=(typeof state!=='undefined' && state.isOffline) || (typeof navigator!=='undefined' && navigator.onLine===false);
    try{
      if(!(window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform())){
        showToast('هذه الميزة متاحة داخل تطبيق أندرويد فقط.', 'info');
        return;
      }
      const { FileOpener }=window.Capacitor.Plugins || {};
      if(!FileOpener){
        showToast('تعذر فتح الملف بتطبيق آخر (المكوّن غير مثبت).', 'error');
        return;
      }
      let uri=record.nativeUri;
      if(!uri){
        // record.blob already exists locally (that's why the in-app viewer
        // could show it), so writing it to disk needs no internet at all.
        uri=await ivWriteBlobToFilesystem(record.blob, record.name);
        if(uri){ record.nativeUri=uri; ivPersistNativeUri(record.key, uri); }
      }
      if(!uri) throw new Error('تعذر تجهيز الملف للفتح الخارجي.');
      await FileOpener.open({ filePath:uri, contentType:record.type||'application/pdf' });
    }catch(e){
      const msg=String(e?.message||e||'').toLowerCase();
      if(/no activity|no app|not.*found|unable to find|activitynotfound/.test(msg)){
        showToast('لا يوجد تطبيق مثبت لفتح ملف PDF.', 'error');
      }else if(isOffline || (typeof isNetworkError==='function' && isNetworkError(e))){
        showToast('لا يوجد اتصال بالإنترنت', 'error');
      }else{
        console.warn('open with external app failed', e);
        showToast('تعذر فتح الملف بتطبيق آخر.', 'error');
      }
    }
  }
  ...
  downloadMaterial = async function(item) {
    ...
    try {
      ...
      const offline = typeof isNetworkError === 'function' && isNetworkError(e);
      showToast(offline ? 'لا يوجد اتصال بالإنترنت' : (e?.message || 'تعذّر تحميل الملف. سجّل الدخول وتأكد من صلاحية الحساب.'), 'error');
    } finally {
  ...
  async function downloadMemberObject(path, name) {
    ...
    } catch (e) { showToast((typeof isNetworkError === 'function' && isNetworkError(e)) ? 'لا يوجد اتصال بالإنترنت' : (e.message || 'تعذر تحميل الملف.'), 'error'); }
  }
